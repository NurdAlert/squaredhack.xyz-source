...
_declspec(noinline)void hooks::physicssimulate_detour(player_t* player)
{
    ...
    if (!player || player->IsDead() || m_globals()->m_tickcount == m_nSimulationTick || player != g_ctx.local() || !cctx->needsprocessing || m_engine()->IsPlayingDemo() || m_engine()->IsHLTV() || player->m_fFlags() & 0x40)
    {
...
using eye_angles = Vector * (__thiscall*)(void*);
...
void __fastcall hooks::hkBuildTransformations(void* ecx, void* edx, CStudioHdr* hdr, Vector* pos, Quaternion* q, matrix3x4_t* cameraTransform, int bonemask, byte* computed)
{
    static auto oBuildTransformations = hooks::player_hook->get_func_address <BuildTransformations>(190);

    //blah blah

    //Only do this for the localplayer
...
{
    static int* WantedReturnAddress1 = (int*)util::FindSignature(crypt_str("client.dll"), "8B CE F3 0F 10 00 8B 06 F3 0F 11 45 ? FF 90 ? ? ? ? F3 0F 10 55 ?"); //Update Animations X/Y
    static int* WantedReturnAddress2 = (int*)util::FindSignature(crypt_str("client.dll"), "F3 0F 10 55 ? 51 8B 8E ? ? ? ?");                                    //Update Animations X/Y
    static int* WantedReturnAddress3 = (int*)util::FindSignature(crypt_str("client.dll"), "8B 55 0C 8B C8 E8 ? ? ? ? 83 C4 08 5E 8B E5");                       //Retarded valve fix

    static auto oGetEyeAngles = hooks::player_hook->get_func_address <eye_angles>(170);
