...
bool __stdcall hooked_createmove(float sample_time, CUserCmd* m_pcmd, bool& bSendPacket)
{
...
void __stdcall CreateMove(int sequence_number, float input_sample_frametime, bool active, bool& bSendPacket)
{
	...
	CVerifiedUserCmd* pVerified = m_input()->GetVerifiedUserCmd(sequence_number);

	if (!pVerified)
		return;

	bool cmoveactive = hooked_createmove(input_sample_frametime, m_pcmd, bSendPacket);

	pVerified->m_cmd = *m_pcmd;
	pVerified->m_crc = m_pcmd->GetChecksum();

}

//fixed by team-g0df4th3r cuz sunset devs are actually too retarded to do anything (you suck but thx)
