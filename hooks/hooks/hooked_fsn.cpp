...
void __stdcall hooks::hooked_fsn(ClientFrameStage_t stage)
{
	...
	//lagcompensation::get().PVSFix(stage);
	...
	if (stage == FRAME_NET_UPDATE_END)
	{
		...
		if (current_shot != g_ctx.shots.end())
		{
			...
			if (current_shot->latency && !current_shot->hurt_player)
			{
				current_shot->shot_info.should_log = true;
				current_shot->shot_info.result = crypt_str("Death");
				eventlogs::get().add(crypt_str("missed shot due to local death"));
			}

			else if (!current_shot->latency)
			{
				...
				if (!current_shot->hurt_player)
				{
					...
					else if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
					{
						...
						else if (current_shot->shot_info.hitchance == 100 || current_shot->shot_info.backtrack_ticks > 7)
						{
							current_shot->shot_info.result = crypt_str("Prediction");
							eventlogs::get().add(crypt_str("missed shot due to prediction"));
						}
