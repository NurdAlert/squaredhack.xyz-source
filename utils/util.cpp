...
namespace util
{
	...
	void copy_command(CUserCmd* cmd, int tickbase_shift)
	{
		...
		do 
		{
			...
			{
				if (g_ctx.globals.original_forwardmove >= 10.0f)
					cmd->m_forwardmove = cl_forwardspeed->GetFloat();
				else if (g_ctx.globals.original_forwardmove <= -10.0f)
					cmd->m_forwardmove = -cl_forwardspeed->GetFloat();

				if (g_ctx.globals.original_sidemove >= 10.0f)
					cmd->m_sidemove = cl_sidespeed->GetFloat();
				else if (g_ctx.globals.original_sidemove <= -10.0f)
					cmd->m_sidemove = -cl_sidespeed->GetFloat();
			}
