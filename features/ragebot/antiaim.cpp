...
void antiaim::fakeflick(CUserCmd* m_pcmd)
{
	...
	if (m_globals()->m_tickcount % g_cfg.antiaim.flick_tick - 1 == 0)
		g_ctx.globals.tickbase_shift++;

	if (m_globals()->m_tickcount % g_cfg.antiaim.flick_tick == 0)
	{
		flicker = true;
		m_pcmd->m_viewangles.y = m_pcmd->m_viewangles.y + ((int)flip == true ? -90 : 90);
	}
	else
		flicker = false;
}
...
float antiaim::get_yaw(CUserCmd* m_pcmd)
{
	...
	if (final_manual_side == SIDE_RIGHT)
		base_angle += 90.0f;
	if (final_manual_side == SIDE_FORWARD)
		base_angle += 180.0f;
