...
class resolver
{
	...
	bool was_first_lowdelta = false;
	bool was_second_lowdelta = false;
	std::vector<float> saved_lean_amounts;
public:
	void initialize(player_t* e, adjust_data* record, const float& goal_feet_yaw, const float& pitch);
	void reset();
	void resolve_yaw();
	float resolve_pitch();
	bool can_shoot_antiaim(player_t* ent);
...
public:
	...
	void store_data(player_t* e, bool store = true)
	{
		...
		goal_feet_yaw = player->get_animation_state()->m_flGoalFeetYaw;
	}
