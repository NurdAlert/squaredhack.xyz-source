...
void local_animations::run(ClientFrameStage_t stage)
{
	...
	if (!fakelag::get().condition && key_binds::get().get_key_bind_state(20))
	{
		...
		if (g_cfg.misc.funny_animations)
		{
			g_ctx.local()->m_flPoseParameter()[16] = 1;
			g_ctx.local()->m_flPoseParameter()[17] = 1;
			g_ctx.local()->m_flPoseParameter()[8] = 0;
			g_ctx.local()->m_flPoseParameter()[14] = 1;
		}
